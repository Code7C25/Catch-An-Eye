...
def calibrate(face_mesh, cap):
    ...
    try:
        ...
        print("Por favor, haz clic en la ventana de calibración si no está en primer plano.")
            ...
            while not exit_program.is_set():
                ...
                if results.multi_face_landmarks:
                    ...
                    if current_mode in [OperatingMode.CURSOR, OperatingMode.DRAG]:
                        ...
                        # --- LÓGICA DE SCROLL CORREGIDA Y MÁS SENSIBLE ---
                        min_nose_y = affine_min_gaze[1]
                        max_nose_y = affine_max_gaze[1]
                        y_range = max_nose_y - min_nose_y

                        if y_range > 1: # Evitar división por cero
                            # Normalizar la posición Y de la nariz de 0 a 1 dentro del rango calibrado
                            norm_y = (nose_y - min_nose_y) / y_range
                            # Centrar el valor en 0 (de -0.5 a 0.5)
                            centered_y = norm_y - 0.5
                            # Multiplicar por un factor de sensibilidad. Aumenta este valor para un scroll más rápido.
                            scroll_speed = -int(centered_y * 50) 
                            
                            if abs(scroll_speed) > 2:
                    ...
                    if right_wink and not left_wink:
                        if not is_right_winking:
                            is_right_winking = True
                            right_wink_start_time = time.time()
                    else:
                        if is_right_winking:
                            ...
                            if current_mode == OperatingMode.DRAG:
                                ...
                                if wink_duration < LONG_WINK_THRESHOLD:
                                    ...
                                    if current_time - last_right_wink_time < DOUBLE_CLICK_INTERVAL:
